...
def compute_visual_bounding_box(root_obj):
    ...
    visual_objs = []
    if root_obj.type == 'MESH' and not root_obj.name.startswith("UCX_") and not "-convcol" in root_obj.name:
        visual_objs.append(root_obj)

    for child in root_obj.children_recursive:
        if child.type == 'MESH' and not child.name.startswith("UCX_") and not "-convcol" in child.name:
...
def verify_asset(asset_name, root_obj=None, config=None):
    ...
    if root_obj is None:
        ...
        if not root_obj:
            ...
            for obj in bpy.context.scene.objects:
                if obj.name.startswith(asset_name) and not obj.name.startswith("UCX_") and not "-convcol" in obj.name:
                    root_obj = obj
                    break
    ...
    visual_objs = [root_obj] if root_obj.type == 'MESH' and not root_obj.name.startswith("UCX_") else []
    for c in root_obj.children_recursive:
        if c.type == 'MESH' and not c.name.startswith("UCX_") and not "-convcol" in c.name:
    ...
    col_objs = [
        ...
        if obj.name.startswith(f"UCX_{asset_name}") or obj.name.endswith("-convcol") or (obj.parent == root_obj and obj.name.startswith("UCX_"))
    ]
    report["collision_count"] = len(col_objs)
    report["collision_pass"] = len(col_objs) > 0
    if not report["collision_pass"]:
        report["warnings"].append("No collision hulls (UCX_* / -convcol) detected.")
