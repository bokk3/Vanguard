...
func _update_mode_ui() -> void:
	if is_cloud_mode:
		...
		if cloud_toggle_btn: cloud_toggle_btn.text = "⮌ SWITCH TO LOCAL DOSSIER"
		...
		if cloud_toggle_btn: cloud_toggle_btn.text = "🌐 SWITCH TO CLOUD NETWORK"
		if auth_btn: auth_btn.text = "🚀 AUTHENTICATE & ENTER HANGAR"
		if status_label:
...
func _on_toggle_cloud_mode() -> void:
	is_cloud_mode = not is_cloud_mode
	_update_mode_ui()
...
func _on_auth_pressed() -> void:
	...
	var auth_mgr = get_node_or_null("/root/AuthManager")
	if not auth_mgr:
		# Fallback if singleton somehow not loaded
		hide()
